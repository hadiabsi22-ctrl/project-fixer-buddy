...
const TheoryForm = () => {
  ...
  const fetchTheory = async () => {
    try {
      ...
      toast.error("حدث خطأ أثناء تحميل النظرية");
      navigate("/admin/theories");
    } finally {
      setIsFetching(false);
    }
  };
    ...
    try {
      ...
      if (isEditing) {
        const { error } = await supabase
          ...
          .eq("id", id);
        if (error) throw error;
        toast.success("تم تحديث النظرية بنجاح");
      } else {
        const { error } = await supabase.from("theories").insert(theoryData);
        if (error) throw error;
        toast.success("تم إنشاء النظرية بنجاح");
      }
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-foreground">
              {isEditing ? "تعديل النظرية" : "نظرية جديدة"}
            </h2>
            <p className="text-muted-foreground">
              {isEditing ? "تعديل بيانات النظرية" : "إضافة نظرية جديدة للموقع"}
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <Card className="gaming-card">
            ...
            <CardContent className="space-y-4">
              ...
              <RichContentEditor
                ...
                label="المحتوى"
                placeholder="اكتب محتوى النظرية الكامل هنا..."
              />
            </CardContent>
          </Card>
          ...
          <Card className="gaming-card">
            ...
            <CardContent>
              <div className="flex items-center justify-between">
                <div>
                  <Label htmlFor="is_published">نشر النظرية</Label>
                  <p className="text-sm text-muted-foreground">
                    اجعل النظرية مرئية للجميع
                  </p>
                </div>
                <Switch
                  id="is_published"
